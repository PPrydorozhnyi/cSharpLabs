...
namespace cast2
{
  class Sudoku
  {
    ...
    public static void Start()
    {
      ...
      int err = Error(soln);
      Console.WriteLine(err == 0 ? "Success \n" : "Did not find optimal solution \n");

      Console.ReadLine();

    } // Main()

    private static int[][] Solve(int[][] problem, int numOrganisms, int maxEpochs, int maxRestarts)
    {
    ...
    private static int[][] SolveEvo(int[][] problem, int numOrganisms, int maxEpochs)
    {
    ...
    private static int[][] RandomMatrix(int[][] problem)
    {
    ...
    private static int Block(int r, int c)
    {
      if (r >= 0 && r <= 2 && c >= 0 && c <= 2)
        return 0;
    ...
    private static int[][] NeighborMatrix(int[][] problem, int[][] matrix)
    {
    ...
    private static int[][] MergeMatrices(int[][] m1, int[][] m2)
    {
  ...
  public class Organism
  {
    ...
    {
      this.type = type;
      this.matrix = Sudoku.DuplicateMatrix(m);
      this.error = error;
      this.age = age;
    }
  }
